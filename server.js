...
function buildFilters(req, tipo) {
  ...
  if (req.query.dimension) {
    // IMPORTANTE: Usar búsqueda flexible para manejar diferencias de espacios
    // Normalizar espacios múltiples a uno solo
    const normalizedDimension = req.query.dimension.replace(/\s+/g, ' ').trim();
    
    if (isVentas) {
      // En VENTAS buscar en dimensionvalor (con normalización de espacios)
      filters.push(`REGEXP_REPLACE(dimensionvalor, '\\s+', ' ', 'g') = $${paramCount++}`);
    } else {
      // En COMPRAS buscar en empresa (con normalización de espacios)
      filters.push(`REGEXP_REPLACE(empresa, '\\s+', ' ', 'g') = $${paramCount++}`);
    }
    params.push(normalizedDimension);
  }
...
app.get('/api/filtros/dimensiones-ventas', async (req, res) => {
  try {
    const query = `
      SELECT DISTINCT 
        REGEXP_REPLACE(dimensionvalor, '\\s+', ' ', 'g') as nombre 
      FROM corporacion_analisisfacturadeventas2 
...
app.get('/api/filtros/dimensiones-compras', async (req, res) => {
  try {
    const query = `
      SELECT DISTINCT 
        REGEXP_REPLACE(empresa, '\\s+', ' ', 'g') as nombre 
      FROM corporacion_analisisfacturadecompras2 
