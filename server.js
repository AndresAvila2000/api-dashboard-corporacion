...
app.get('/api/ventas/stats', async (req, res) => {
  try {
    ...
    const query = `
      SELECT 
        COALESCE(SUM(importemonprincipal::numeric), 0) as total_facturado,
        COUNT(*) as cantidad_facturas,
        COUNT(DISTINCT cliente) as clientes_unicos,
        COALESCE(AVG(importemonprincipal::numeric), 0) as promedio_factura
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        SUM(importemonprincipal::numeric) as total
      FROM corporacion_analisisfacturadeventas2
      ${whereClause}
...
app.get('/api/compras/stats', async (req, res) => {
  try {
    ...
    const query = `
      SELECT 
        COALESCE(SUM(importe_mon_principal::numeric), 0) as total_compras,
        COUNT(*) as cantidad_facturas,
        COUNT(DISTINCT proveedor) as proveedores_unicos,
        COALESCE(AVG(importe_mon_principal::numeric), 0) as promedio_compra
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        SUM(importe_mon_principal::numeric) as total
      FROM corporacion_analisisfacturadecompras2
      ${whereClause}
